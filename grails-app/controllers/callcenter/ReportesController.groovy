...
class ReportesController {
	...
	def bitacoraAdicional(){
		...
		if(params.fechas){
			...
			String sqlGestion = "from Clientes where fechaGestion between :fechaInicio and :fechaFin"
			...
			def adicionalesGestionList = Clientes.executeQuery(sqlGestion, condicionesGestion)
			...
			workbook.createSheet("Reporte Gestion", 1)
			workbook.createSheet("GXC_UIO_MARCACIONES_" + fecha, 2)
			...
			WritableSheet sheetHistorial = workbook.getSheet(2)
			String[] headers = ['ID', 'NOMBRE SOCIO ADICIONAL', 'NOMBRE EN PLASTICO', 'CEDULA SOCIO ADICIONAL', 'CUPO SOCIO ADICIONAL', 'CORREO ADICIONAL', 'NUMERO CELULAR ADICIONAL',
								...
								'ACEPTACION ARCOTEL', 'PREGUNTA 1', 'PREGUNTA 2', 'PREGUNTA 3', 'PREGUNTA 4', 'TIPO DIRECCION', 'ACEPTACION VEHICULAR', 'MOTIVO NO ACEPTA', 'OBSERVACION',
								'','OTORGA DATOS','VALIDACION DATOS','NOTIFICACION CONSUMO']
			String[] headersGestion = ['BASE',	'CEDULA',	'CLIENTE',	'PRODUCTO_VENDIDO',	'CUPO',	'FECHA_CARGA',	'OBSERVACION',	'FECHA_INICIO',	'FECHA FIN',
									   'CAMPANA',	'EASYCODE',	'TELEFONO LLAMADA',	'AGENTE',	'CORREO',	'ESTADO TELEFONICO',	'HORA LLAMADA',	'GESTION',
									   'TIPO GESTION',	'SUBTIPO GESTION',	'SUBTIPO GESTION 1',	'OBSERVACION ADICIONAL',	'GESTION FINAL - ABSOLUTA',
									   'CONTEO DE MARCACIONES',	'SEGURO',	'ESTADO DE CUENTA VIRTUAL',	'BIENVENIDA',	'CONSOLIDADO NP',
									   'PRODUCTO A OFERTAR','NUMERO ADICIONALES', '', 'ESTADO GESTION', 'ACEPTACION VEHICULAR', 'MOTIVO NO ACEPTA', 'OBSERVACION','OFERTA DE VALOR']
			String[] headersHistorial = ['ID_MES',	'ID_IDENTIFICACION',	'CEDULA',	'CLIENTE',	'CODIGO_BASE',	'NOMBRE_BASE',
									   'ID_TIPO_BASE',	'EXTERNAL_TOKEN',	'FECHA_ENVIO BASE',	'NUMERO_TELEFONO',	'FECHA_LLAMADA',
									   'HORA_INICIO LLAMADA',	'HORA FIN LLAMADA',	'TIEMPO DE GESTION',	'CANAL_ENVIO',	'MARCA',
									   'ID_GESTION',	'FECHA_INICIO_VIGENCIA',	'FECHA_FIN_VIGENCIA',	'ODV',	'DESC_CAMPAIGNS',
									   'STATUS_TELEFONO',	'AGENTE',	'USUARIO_AGENTE',	'CIUDAD',	'SEGMENTO',	'GESTION_CANAL', 'DESCRIPCION GESTION_CANAL',
									   'ULTIMO_CALLTYPE',	'CONTACTABILIDAD',	'RESULTADO',	'MOTIVO', 'DESCRIPCION_CALLTYPE',	'PRODUCTO VENDIDO ACEPTADO POR EL SOCIO',
									   'OTROS',	'ACEPTA COMBO', 'MOTIVO DE NO ACEPTA COMBO', "", "CLIENTE ID", "FECHA Y HORA HISTORIAL"]
			...
			ExcelUtils.addCells(headersHistorial, sheetHistorial, 0, Colour.GOLD, Alignment.LEFT, VerticalAlignment.CENTRE, cellFont, Border.ALL, BorderLineStyle.HAIR)
...
//				campos[0] = formatearFecha(cli.fechaGestion.toString())//fechaGestion
			...
			//	println(cli.id)
				...
				campos[41] = ""
				if (adi.validacionDatos == 'SI OTORGA DATOS'){
					campos[42] = "SI"
				}else{
					campos[42] = "NO"
				}

				campos[43] = adi.validacionDatos
				campos[44] = adi.notificacionConsumo
				ExcelUtils.addCells(campos, sheet, i+1, Colour.WHITE, Alignment.LEFT, VerticalAlignment.CENTRE, cellFont2, null, null)
			}

			for(int j = 0; j < adicionalesGestionList.size(); j++){
				String[] campos2 = new String[headersGestion.length]
				Clientes cli = adicionalesGestionList.get(j)
				String producto = ""
				String correo = ""
				String numeroAdicionales = ""
				 if(cli.subestadoGestion.enableManagement.equals(true)){
					 Clientes idCliente = Clientes.findById(cli.id.toString().toLong())
				...
				campos2[0] = cli.nombreArchivoCliente.toUpperCase()
				campos2[1] = cli.identificacion
				campos2[2] = cli.apellidos.toUpperCase() + " " + cli.nombres.toUpperCase()
				campos2[3] = producto
				campos2[4] = ""
				campos2[5] = cli.fechaCarga.toString()
				campos2[6] = cli.observaciones
				campos2[7] = cli.fechaInicioLlamada
				campos2[8] = cli.fechaFinLlamada
				campos2[9] = 'ADC_' + cli.productoVender
				campos2[10] = cli.id
				campos2[11] = cli.telefonoContactado

				campos2[12] = cli.usuario.usuario.toUpperCase()
				campos2[13] = cli.email.toUpperCase()
				campos2[14] = cli.estadoTelefonoContactado.toUpperCase()
				campos2[15] = cli.horaLlamada
				campos2[16] = cli.subestadoGestion.nombre.toUpperCase()
				if(cli.subSubEstado == null){
					campos2[17] = cli.subSubEstado
				}else{
					campos2[17] = cli.subSubEstado.toUpperCase()
				}
				campos2[18] = "SIN GESTION"
				campos2[19] = "SIN GESTION"
				campos2[20] = cli.observaciones
				campos2[21] = cli.subestadoGestion.nombre.toUpperCase()
				campos2[22] = cli.intentos
				campos2[23] = cli.seguroDesgravamen
				campos2[24] = ""
				campos2[25] = ""
				campos2[26] = ""
				campos2[27] = cli.productoVender
				campos2[28] = numeroAdicionales
				campos2[29] = ""
				campos2[30] = cli.estadoGestion
				campos2[31] = cli.aceptacionCreditoVehicular
				campos2[32] = cli.motivoNoAceptaVehicular
				campos2[33] = cli.observacionesNoAcepta
				campos2[34] = cli.propuestaValorElegida

				ExcelUtils.addCells(campos2, sheetGestion, j+1, Colour.WHITE, Alignment.LEFT, VerticalAlignment.CENTRE, cellFont2, null, null)
			}

			for(int k = 0; k < historialGestionList.size(); k++){
				String[] campos = new String[headersHistorial.length]
				Historial hist = historialGestionList.get(k)
				Clientes cli = hist.cliente
				campos[0] = hist.fechaGestion.toString().substring(0,8).replace("-","")
				campos[1] = "C"
				campos[2] = cli.identificacion
				campos[3] = cli.apellidos.toUpperCase() + " " + cli.nombres.toUpperCase()
				campos[4] = cli.base
				campos[5] = cli.nombreArchivoCliente
				campos[6] = "ADI"
				campos[7] = ""
				campos[8] = "01/06/2020"
				campos[9] = hist.telefonoContactado
				campos[10] = formatearFecha(hist.fechaGestion.toString())
				if (hist.fechaInicioLlamada != null){
					campos[11] = hist.fechaInicioLlamada.substring(11,19)
					campos[12] = hist.fechaFinLlamada.substring(11,19)
				}else{
					campos[11] = hist.fechaInicioLlamada
					campos[12] = hist.fechaFinLlamada
				}

				campos[13] = ""
				campos[14] = "GXC"
				campos[15] = cli.productoVender
				campos[16] = ""
				campos[17] = cli.fechaInicio
				campos[18] = cli.fechaVigente
				campos[19] = cli.grupo
				campos[20] = "ADC_" + cli.productoVender
				campos[21] = hist.estadoTelefonoContactado
				campos[22] = hist.nombreVendedor
				campos[23] = hist.usuario.usuario
				campos[24] = cli.ciudad
				campos[25] = cli.segmento
				campos[26] = hist.subestadoGestion.nombre
				campos[27] = hist.motivoNoDesea
				println(cli.id)
				campos[28] = cli.subestadoGestion.nombre
				campos[29] = ""
				campos[30] = ""
				campos[31] = ""
				campos[32] = ""
				...
				campos[33] = producto
				campos[34] = cli.seguroDesgravamen
				campos[35] = ""
				campos[36] = ""
				campos[37] = ""
				campos[38] = cli.id
				campos[39] = hist.fechaGestion.toString()
				ExcelUtils.addCells(campos, sheetHistorial, k+1, Colour.WHITE, Alignment.LEFT, VerticalAlignment.CENTRE, cellFont2, null, null)
			}
	...
	def ventas(){
		...
		if(params.fechas){
			...
			String[] headers = ['ID', 'NOMBRE SOCIO ADICIONAL', 'NOMBRE EN PLASTICO', 'CEDULA SOCIO ADICIONAL', 'CUPO SOCIO ADICIONAL', 'CORREO ADICIONAL', 'NUMERO CELULAR ADICIONAL',
								...
								'ACEPTACION VEHICULAR', 'MOTIVO NO ACEPTA', 'OBSERVACION', 'MONTO AHORRO PAD', 'CODIGO OTP PAD', '', 'ID SISTEMA', 'NOMBRE', 'FECHA GESTION','CREADAS / NO CREADAS','IMPUTABLE','DETALLE IMPUTABLE','FECHA ENVIO CREACION']
...
//				campos[0] = formatearFecha(cli.fechaGestion.toString())//fechaGestion
				...
				campos[47] = adi.creadasNocreadas
				campos[48] = adi.imputable
				campos[49] = adi.detalleImputable
				campos[50] = adi.fechaEnvioCreacion
				ExcelUtils.addCells(campos, sheet, i+1, Colour.WHITE, Alignment.LEFT, VerticalAlignment.CENTRE, cellFont2, null, null)
			}
	...
	def indicadoresGestion(){
		...
		if(params.fechas){
			...
			def CU3PorUsuario = Clientes.executeQuery("select nombreVendedor, count(*), substring(fechaGestion,1,10) from Historial where estadoGestion = 'CONTACTADO' and cliente.deudaProtegida = 'SI OFERTAR PLAN DEUDA PROTEGIDA' and fechaGestion between :fechaInicio and :fechaFin group by nombreVendedor, substring(fechaGestion,1,10) order by nombreVendedor", [fechaInicio: fechaInicio, fechaFin: fechaFin])
